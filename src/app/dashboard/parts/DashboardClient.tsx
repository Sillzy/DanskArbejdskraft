...
function rangeDays(start: Date, end: Date) { const out: Date[] = []; const cur = new Date(start); cur.setHours(0,0,0,0); while (cur <= end) { out.push(new Date(cur)); cur.setDate(cur.getDate() + 1); } return out; }
/** ISO Week Number (1..53) */
function getISOWeek(date: Date) { const d = new Date(date); const day = (d.getDay() + 6) % 7; d.setDate(d.getDate() - day + 3); const firstThursday = new Date(d.getFullYear(), 0, 4); const dayDiff = (d.getTime() - firstThursday.getTime()) / 86400000; return 1 + Math.round(dayDiff / 7); }
...
function RegisterTimeModal({
  ...
  async function submit() {
    ...
    if (!(endLocal > startLocal)) { setErr('End time must be after start time.'); return; }
...
function UploadPhotoModal({
  open, onClose, workplaceId,
}: { open: boolean; onClose: (uploaded?: boolean) => void; workplaceId: string | null; }) {
  const [file, setFile] = React.useState<File | null>(null);
  const [err, setErr] = React.useState<string | null>(null);
  const [uploading, setUploading] = React.useState(false);
  const inputRef = React.useRef<HTMLInputElement>(null);
  React.useEffect(() => { if (!open) { setFile(null); setErr(null); setUploading(false); } }, [open]);
  const isMobile = React.useMemo(() => typeof navigator !== 'undefined' && /Android|iPhone|iPad|iPod/i.test(navigator.userAgent), []);
  ...
  async function doUpload() {
    ...
    try {
      ...
      const { error } = await supabase.storage.from('workplace-photos').upload(path, file, {
  ...
  return (
    <div className="fixed inset-0 z-[90]">
      ...
      >
        ...
        {err && <div className="mt-4 rounded-md bg-red-50 px-4 py-3 text-base text-red-700">{err}</div>}
        <div className="mt-5">
          <input ref={inputRef} type="file" accept="image/*" {...(isMobile ? { capture: 'environment' as any } : {})} className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          <button type="button" onClick={() => inputRef.current?.click()} className="rounded-lg border px-4 py-2 text-sm hover:bg-slate-50">
...
export default function DashboardClient({
  ...
  React.useEffect(() => {
    ...
    async function fetchLatest() {
      const { data, error } = await supabase
        ...
        .eq('user_id', userId)
        .single();
      if (!cancelled && !error && data?.profile_status) setProfileStatus(data.profile_status as any);
    }
    fetchLatest();
    const channel = supabase
      .channel(`profile_status_${userId}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles', filter: `user_id=eq.${userId}` },
        (payload) => { const next = (payload.new as any)?.profile_status; if (next) setProfileStatus(next); })
      .subscribe();
  ...
  const loadCalendarMonth = React.useCallback(async (month: Date) => {
    ...
    if (!error && data) {
      for (const r of data as any[]) {
        const mins = Math.max(0, Math.round((new Date(r.ended_at).getTime() - new Date(r.started_at).getTime()) / 60000) - (r.break_minutes || 0));
        const d = new Date(r.started_at);
        const key = ymd(d);
        map.set(key, (map.get(key) ?? 0) + mins);
      }
    }
    setPerDayCal(map); setLoadingCal(false);
  }, [userId]);

  React.useEffect(() => { loadCalendarMonth(calMonth); }, [calMonth, loadCalendarMonth]);

  function prevMonth() { if (calMonth <= MIN_CAL_MONTH) return; setCalMonth(startOfMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() - 1, 1))); }
  function nextMonth() { if (calMonth >= NOW_MONTH) return; setCalMonth(startOfMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() + 1, 1))); }
  ...
  const thisWeekMin = React.useMemo(() => {
    ...
    let total = 0;
    for (const day of rangeDays(start, end)) total += perDay.get(ymd(day)) ?? 0;
    return total;
  }, [perDay]);

  type WeekRow = { isoWeek: number; label: string; spanText: string; totalMin: number; weekendMin: number; otFirst3: number; otAfter3: number; };
  const weeks5: WeekRow[] = React.useMemo(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      rows.push({ isoWeek: getISOWeek(start), label: `Week ${getISOWeek(start)}`, spanText, totalMin: total, weekendMin: weekend, otFirst3, otAfter3 });
    }
    return rows;
  }, [perDay]);
  ...
  async function deleteWorkplace(workplaceId: string, _createdAt: string) {
    try {
      ...
      if (!res.ok) {
        const j = await res.json().catch(() => ({} as any));
        alert(j?.error || 'Failed to remove workplace from your dashboard.');
        return;
      }
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border bg-white p-5">
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-800">
              ...
              <Row label="Birthdate" value={profile?.birthday ? new Date(profile?.birthday).toLocaleDateString('en-GB') : '—'} />
              <Row label="Address" value={profile?.address ?? '—'} />
              <Row label="City" value={profile?.city ?? '—'} />
              <Row label="Post code" value={profile?.postal_code ?? '—'} />
              <Row label="Phone" value={(profile?.phone_country || profile?.phone_number) ? `${profile?.phone_country ?? ''} ${profile?.phone_number ?? ''}`.trim() : '—'} icon={<Phone className="h-4 w-4 text-slate-500" />} />
            </div>
            <div className="mt-4">
              <Link href="/dashboard/profile" className="inline-flex items-center rounded-lg border px-4 py-2 text-sm hover:bg-slate-50">Edit Profile</Link>
            </div>
            <div className="mt-4 rounded-lg bg-slate-50 px-3 py-2 text-sm">
              Profile status:{' '}
              <span className="font-medium">
                {profileStatus === 'approved' ? 'approved' : profileStatus === 'rejected' ? 'rejected' : 'Under Review'}
              </span>
            </div>
          </section>

          <section className="rounded-2xl border bg-white p-5">
            <div className="flex items-center gap-2"><Clock className="h-5 w-5 text-slate-700" /><h2 className="text-lg font-semibold">Overview / Statistics</h2></div>
                ...
                {weeks5.map((w, idx) => (
                  <div key={idx} className="rounded-xl border p-3">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-medium">{w.label} <span className="text-slate-500">({w.spanText})</span></div>
                      <div className="text-sm font-semibold">{hhmm(w.totalMin)}</div>
                    </div>
                    <div className="mt-2 grid grid-cols-4 gap-2 text-xs">
                      <div className="rounded-lg bg-slate-50 p-2 text-center"><div className="text-slate-500">Total Hours</div><div className="font-semibold">{hhmm(w.totalMin)}</div></div>
                      <div className="rounded-lg bg-emerald-50 p-2 text-center"><div className="text-slate-600">Weekend Hours</div><div className="font-semibold">{hhmm(w.weekendMin)}</div></div>
                      <div className="rounded-lg bg-blue-50 p-2 text-center"><div className="text-slate-600">OT First 3 Hours</div><div className="font-semibold">{hhmm(w.otFirst3)}</div></div>
                      <div className="rounded-lg bg-indigo-50 p-2 text-center"><div className="text-slate-600">OT After 3 Hours</div><div className="font-semibold">{hhmm(w.otAfter3)}</div></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-7 gap-2 text-center">
                    {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((w) => <div key={w} className="text-xs text-slate-500">{w}</div>)}
                    {(() => { const first = new Date(calStart); const weekday = (first.getDay() + 6) % 7; return Array.from({ length: weekday }).map((_, i) => <div key={`lead-${i}`} />); })()}
                    {days.map((d) => { const key = ymd(d); const mins = perDayCal.get(key) ?? 0;
                      return (
                        <div key={key} className="rounded-lg border p-2 flex flex-col items-center justify-center">
        ...
        <section className="lg:col-span-2 space-y-6">
          ...
          {profileStatus === 'approved' ? (
            <div className="rounded-2xl border bg-white p-5">
              ...
              <p className="mt-1 text-sm text-slate-600">Tap a box to add it to your dashboard. Only workplaces marked <em>Active</em> by Admin are shown.</p>
              <AvailableWorkplacesGrid
                ...
                onAdded={async (id) => {
                  const { data } = await supabase
                    ...
                    .eq('id', id)
                    .single();
                  if (data) setWps((prev) => [data as any, ...prev.filter((p) => p.id !== id)]);
                }}
              />
            </div>
          ) : (
